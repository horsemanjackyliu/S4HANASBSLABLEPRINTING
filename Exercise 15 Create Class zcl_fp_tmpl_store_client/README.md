# Exercise 15: Create Class `zcl_fp_tmpl_store_client`

In this exercise you will create the supporting ABAP objects needed to interact with the **SAP Forms Template Store REST API**. You will build:

1. A **message class** (`Z_TMPL_STORE`) for error messages
2. An **exception class** (`ZCX_FP_TMPL_STORE_ERROR`) that uses those messages
3. A **client class** (`ZCL_FP_TMPL_STORE_CLIENT`) that wraps the Template Store HTTP API

---

## Step 1: Create Message Class `Z_TMPL_STORE`

In Eclipse ADT, open the **New** wizard and create a new Message Class.

![Create message class - step 1](img/image-1.png)

![Create message class - step 2](img/image-2.png)

| Field | Value |
|-------|-------|
| Name | `Z_TMPL_STORE` |
| Description | `Z_TMPL_STORE` |

Click **Finish**.

![Message class messages](img/image-3.png)

Add the following three messages:

| Number | Text |
|--------|------|
| `001` | `Setup not complete cannot initiate http client (check arrangement + dest)` |
| `002` | `Remote service raised unexpected error: &&1 : &&2` |
| `003` | `HTTP client raised error: &&1` |

**Save** the message class before continuing.

---

## Step 2: Create Exception Class `ZCX_FP_TMPL_STORE_ERROR`

This exception class inherits from `CX_STATIC_CHECK` and maps each message in `Z_TMPL_STORE` to a named constant, making it easy to raise typed exceptions throughout the application.

In Eclipse ADT, create a new ABAP Class named `ZCX_FP_TMPL_STORE_ERROR`.

![Create exception class - step 1](img/image-4.png)

![Create exception class - step 2](img/image.png)
...
**Save** (`Cmd+S`) and **Activate** (`Cmd+F3`) the class.

---

## Step 3: Create Client Class `ZCL_FP_TMPL_STORE_CLIENT`

This class provides a typed ABAP interface over the SAP Forms Template Store REST API (`/v1/forms/...`). It supports both **Communication Arrangement** and **Cloud Destination** as the authentication mechanism.

In Eclipse ADT, create a new ABAP Class named `ZCL_FP_TMPL_STORE_CLIENT`.

![Create client class - step 1](img/image-5.png)

![Create client class - step 2](img/image-6.png)

| Field | Value |
|-------|-------|
| Name | `zcl_fp_tmpl_store_client` |
...
You have now created all three objects required for Template Store integration:

| Object | Type | Purpose |
|--------|------|---------|
| `Z_TMPL_STORE` | Message Class | Error message texts |
| `ZCX_FP_TMPL_STORE_ERROR` | Exception Class | Typed error handling |
| `ZCL_FP_TMPL_STORE_CLIENT` | ABAP Class | HTTP client for Template Store REST API |

`ZCL_FP_TMPL_STORE_CLIENT` is called in Exercise 11 by the `render` method in the behavior implementation class `ZBP_OBJ_DN` to retrieve the XDP label template before generating the PDF.
